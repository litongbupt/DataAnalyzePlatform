...
cllog= foreach filter_cllog generate timestamp, uuid,(jhid is null ? '0' : jhid) as clljhid, UrlDecode(pbquery,'gbk') as cllquery, (abtestid is null? 0 : abtestid) as cllabtestid, SUBSTRING(manualtime,12,14) as cllhour,SUBSTRING(manualtime,0,11) as cllday, uigs_cl_clickinfo , uigs_cl_pos as cllvrpos;
...
endclick_log = foreach filter_group_log generate (REGEX_EXTRACT(uigs_cl_clickinfo , 'sogou_vr_[0-9]+_(.*)', 1) is null ? 'title' :REGEX_EXTRACT(uigs_cl_clickinfo , 'sogou_vr_[0-9]+_(.*)', 1)) as cllickid, (REGEX_EXTRACT(uigs_cl_clickinfo , 'sogou_vr_(.*)_.*', 1) is null ? REGEX_EXTRACT(uigs_cl_clickinfo , 'sogou_vr_(.*)', 1): REGEX_EXTRACT(uigs_cl_clickinfo , 'sogou_vr_(.*)_.*', 1)) as cllvrid,cllquery,cllabtestid,cllhour,cllvrpos,cllday,clljhid;
