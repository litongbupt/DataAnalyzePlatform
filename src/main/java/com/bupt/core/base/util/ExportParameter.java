...
public class ExportParameter{
	...
	private ExportParameter(){
		throw new AssertionError();
	}
	
	private static Map<String, ResouInfoDTO> exportInfos = new HashMap<String, ResouInfoDTO>();
	
	public static Map<String, ResouInfoDTO> getExportInfos(){
		return exportInfos;
	}
	...
	static{
		...
		try {
			localDocument = localReader.read(ExportParameter.class.getClassLoader().getResourceAsStream("com/bupt/config/export_info.xml"));
		...
		for(Iterator it = resouInfos.elementIterator("resou-info"); it.hasNext();){
			...
			ExportParameter.exportInfos.put(name, ridto);
