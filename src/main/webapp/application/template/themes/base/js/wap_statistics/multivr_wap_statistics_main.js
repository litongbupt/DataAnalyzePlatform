$(function(){
	...
	//主页面表格
	$("#modelList").jqGrid({
	    url:golbal_context+"/wap_statistics/list.do",
	    datatype: "local",
	    mtype: "GET",
	    width:"100%",
	    height:"auto",
...
function importInfo(){
	//加载页面
 	$( "#importDialog" ).load(golbal_jsp_position+'/wap_statistics/multivr_wap_statistics_import_diaog.jsp', function(data){
		//表单验证注册
		$("#importUserForm").validate({
	    	...
	    	submitHandler:function(){ //验证通过后调用此函数
	    		...
	    		$('#importUserForm').ajaxSubmit({
	    			url : golbal_context+"/wap_statistics/validateExcel.do",
